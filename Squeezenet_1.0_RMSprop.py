...
def train(epoch):
    ...
    train_correct    = 0
    train_total      = 0
    optimizer  = optim.RMSprop(net.parameters(), lr=lrt, momentum=0.9, weight_decay=5e-4)
    
    print('Sqnet_1x_v1.0_RMSprop Training Epoch: #%d, LR: %.4f'%(epoch, lrt))
...
#             'optimizer_state_dict': optimizer.state_dict()
        ...
        if not os.path.isdir('./checkpoint/Sqnet_1x_v1.0_RMSprop'):
            os.makedirs('./checkpoint/Sqnet_1x_v1.0_RMSprop')
        torch.save(state, './checkpoint/Sqnet_1x_v1.0_RMSprop/Sqnet_1x_v1.0_RMSprop_Cifar10.ckpt')
