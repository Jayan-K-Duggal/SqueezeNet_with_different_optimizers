...
def train(epoch):
    ...
    train_correct    = 0
    train_total      = 0
    optimizer  = optim.Adadelta(net.parameters(), lr=lrt, weight_decay=5e-4)
    
    print('Sqnet_1x_v1.0_Adadelta Training Epoch: #%d, LR: %.4f'%(epoch, lrt))
...
#             'optimizer_state_dict': optimizer.state_dict()
        ...
        if not os.path.isdir('./checkpoint/Sqnet_1x_v1.0_Adadelta'):
            os.makedirs('./checkpoint/Sqnet_1x_v1.0_Adadelta')
        torch.save(state, './checkpoint/Sqnet_1x_v1.0_Adadelta/Sqnet_1x_v1.0_Adadelta_Cifar10.ckpt')
